    ...
    <form ref={formRef} className="modal-card" onSubmit={(event) => { event.preventDefault(); onSave() }}>
      ...
      {!provider ? <div className="modal-body provider-step"><ul className="kind-picker large" aria-label="来源类型">{providerList.map((item) => <li key={item.kind}><button type="button" onClick={() => choose(item.kind)}><item.icon size={22} /><span><strong>{item.label}</strong><small>{item.description}</small></span></button></li>)}</ul></div> : <>
        <div className="modal-body">
          ...
          {draft.kind === 'host' && <>
            <label><span>检查路径</span><textarea required value={draft.hostPaths} onChange={change(setDraft, 'hostPaths')} /><small>每行一个绝对路径；使用 Argus 账户的实际可见范围。</small></label>
            <label><span>关键服务</span><textarea value={draft.hostUnits} onChange={change(setDraft, 'hostUnits')} placeholder="nginx.service" /><small>每行一个 systemd 单元名。只检查，不会启停服务。</small></label>
            <div className="form-grid two">{([['hostDisk', '磁盘使用率阈值（%）'], ['hostInode', 'inode 使用率阈值（%）'], ['hostMemory', '内存使用率阈值（%）'], ['hostLoad', '1 分钟负载阈值']] as const).map(([key, label]) => <label key={key}><span>{label}</span><input required type="number" min="1" max={key === 'hostLoad' ? undefined : 100} step="0.1" value={draft[key]} onChange={(event) => setDraft((current) => ({ ...current, [key]: Number(event.target.value) }))} /></label>)}</div>
          </>}
