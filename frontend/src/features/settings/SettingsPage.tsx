...
const DEFAULT_DIGEST: DigestConfig = { enabled: false, timezone: 'Asia/Shanghai', daily_time: '20:00', item_limit: 50, observation_limit: 5000, notify: true, public_base_url: '', api_summary: true, prompt_id: 'digest', prompt_version: 4 }
...
export function SettingsPage({ status, health, revisions, revisionTotal, busy, analysis, prompts, analysisError, onSaveAnalysis, digest, onSaveDigest, onSavePrompt, advancedKind, advancedJson, onAdvancedKind, onAdvancedJson, onLoadExample, onSaveAdvanced, onRollback }: SettingsPageProps) {
  ...
  return <>
    ...
    <section className="panel settings-panel"><div className="settings-form"><label className="switch-field"><input type="checkbox" checked={digestDraft.api_summary !== false} onChange={(event) => editDigest({ api_summary: event.target.checked })} /><span><strong>日报 API 总结</strong><small>需同时开启分析和远程 API；调用失败会先发布算法日报并在五小时内重试。引用编号对应日报下方条目。</small></span></label><label><span>日报 Prompt</span><select value={`${digestDraft.prompt_id || 'digest'}@${digestDraft.prompt_version || 4}`} onChange={(event) => { const [id, version] = event.target.value.split('@'); editDigest({ prompt_id: id, prompt_version: Number(version) }) }}><option value="digest@4">digest · v4（内置）</option>{prompts.filter((p) => p.prompt_id !== 'digest' || p.version !== 4).map((p) => <option key={`${p.prompt_id}@${p.version}`} value={`${p.prompt_id}@${p.version}`}>{p.prompt_id} · v{p.version}</option>)}</select></label></div></section>
