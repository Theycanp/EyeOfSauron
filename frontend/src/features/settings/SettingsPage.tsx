...
interface SettingsPageProps {
  ...
  onSaveAnalysis: (value: AnalysisConfig) => Promise<boolean | void> | void
  digest?: DigestConfig; onSaveDigest: (value: DigestConfig) => Promise<boolean | void> | void
...
const DEFAULT_ANALYSIS: AnalysisConfig = {
  enabled: false, shadow_mode: true, local_enabled: false, api_enabled: false, api_triage_enabled: false,
  local_base_url: 'http://127.0.0.1:11434/v1', local_model: '', local_model_fallbacks: [], api_base_url: '', api_model: '', api_model_fallbacks: [], api_key_env: '',
...
const DEFAULT_DIGEST: DigestConfig = { enabled: false, timezone: 'Asia/Shanghai', daily_time: '20:00', item_limit: 50, observation_limit: 5000, notify: true, public_base_url: '', api_summary: true, prompt_id: 'digest', prompt_version: 3 }
...
export function SettingsPage({ status, health, revisions, revisionTotal, busy, analysis, prompts, analysisError, onSaveAnalysis, digest, onSaveDigest, onSavePrompt, advancedKind, advancedJson, onAdvancedKind, onAdvancedJson, onLoadExample, onSaveAdvanced, onRollback }: SettingsPageProps) {
  ...
  const apiCallsToday = Number((status.analysis as { api_calls_today?: number } | undefined)?.api_calls_today || 0)
  ...
  return <>
    ...
    <section className="panel settings-panel analysis-settings">
      <div className="panel-header"><div><h2><Sparkles size={17} />信息分析策略</h2><p>算法作为底线；模型只辅助，不能单独提升即时通知等级。</p></div><span className={`status-pill ${draft.enabled ? 'positive' : 'neutral'}`}>{draft.enabled ? draft.local_enabled || draft.api_enabled ? `模型已启用 · 今日 ${apiCallsToday} 次` : '仅算法模式' : '已关闭'}</span></div>
      ...
      <div className="settings-form">
        ...
        <label className="switch-field"><input type="checkbox" checked={Boolean(draft.api_triage_enabled)} disabled={!draft.enabled || !draft.api_enabled} onChange={(event) => editAnalysis({ api_triage_enabled: event.target.checked })} /><span><strong>额外启用逐条 API 分析</strong><small>默认关闭；开启会与日报共享每日调用预算，建议优先留给日报。</small></span></label>
        <label className="switch-field"><input type="checkbox" checked={Boolean(draft.send_full_text)} onChange={(event) => editAnalysis({ send_full_text: event.target.checked })} /><span><strong>发送全文给模型</strong><small>默认仅发送标题摘要；开启后使用已合法保存的全文，仍受输入上限截断，不会绕过付费墙。</small></span></label>
        <div className="dialog-actions"><button className="button primary" disabled={busy || !analysisDirty} onClick={() => { void (async () => { if (await onSaveAnalysis(draft) !== false) setAnalysisDirty(false) })() }}><Save size={16} />保存分析配置</button></div>
      </div>
    </section>

    <section className="panel settings-panel prompt-panel"><div className="panel-header"><div><h2><WandSparkles size={17} />Prompt 版本</h2><p>Prompt 集中保存并版本化；新闻正文始终按不可信数据处理。</p></div><button className="button subtle" onClick={() => openPrompt()}><FileClock size={16} />编辑版本</button></div><div className="prompt-list">{prompts.length ? prompts.map((prompt) => <button key={`${prompt.prompt_id}@${prompt.version}`} className={`prompt-row ${selectedPrompt?.prompt_id === prompt.prompt_id && selectedPrompt.version === prompt.version ? 'active' : ''}`} onClick={() => openPrompt(prompt)}><span><strong>{prompt.prompt_id}</strong><small>v{prompt.version} · {prompt.actor || 'system'}</small></span><Check size={16} /></button>) : <div className="empty-compact"><WandSparkles size={20} />尚未加载 Prompt，使用内置 triage@1</div>}</div>{selectedPrompt !== null && <div className="prompt-editor"><div className="form-grid two"><label><span>Prompt ID</span><input value={promptId} onChange={(event) => setPromptId(event.target.value)} pattern="[a-z][a-z0-9_-]{1,63}" /></label><label><span>版本号</span><input type="number" min="1" value={promptVersion} onChange={(event) => setPromptVersion(Number(event.target.value))} /></label></div><label><span>系统 Prompt</span><textarea rows={8} value={promptText} onChange={(event) => setPromptText(event.target.value)} maxLength={12000} /></label><div className="dialog-actions"><button className="button primary" disabled={busy || !promptId.trim() || !promptText.trim() || !promptDirty} onClick={() => onSavePrompt({ prompt_id: promptId.trim(), version: promptVersion, system_text: promptText })}><Save size={16} />保存 Prompt 版本</button></div></div>}</section>

    <section className="panel settings-panel"><div className="settings-form"><label className="switch-field"><input type="checkbox" checked={digestDraft.api_summary !== false} onChange={(event) => editDigest({ api_summary: event.target.checked })} /><span><strong>日报 API 总结</strong><small>需同时开启分析和远程 API；调用失败会先发布算法日报并在五小时内重试。引用编号对应日报下方条目。</small></span></label><label><span>日报 Prompt</span><select value={`${digestDraft.prompt_id || 'digest'}@${digestDraft.prompt_version || 3}`} onChange={(event) => { const [id, version] = event.target.value.split('@'); editDigest({ prompt_id: id, prompt_version: Number(version) }) }}><option value="digest@3">digest · v3（内置）</option>{prompts.filter((p) => p.prompt_id !== 'digest' || p.version !== 3).map((p) => <option key={`${p.prompt_id}@${p.version}`} value={`${p.prompt_id}@${p.version}`}>{p.prompt_id} · v{p.version}</option>)}</select></label></div></section>
    <section className="panel settings-panel digest-panel"><div className="panel-header"><div><h2><FileClock size={17} />日报与阅读服务</h2><p>聚合普通和重点信息，生成可追溯的版本化摘要。</p></div><span className={`status-pill ${digestDraft.enabled ? 'positive' : 'neutral'}`}>{digestDraft.enabled ? '已启用' : '已关闭'}</span></div><div className="settings-form"><div className="analysis-modes"><label className="switch-field prominent"><input type="checkbox" checked={Boolean(digestDraft.enabled)} onChange={(event) => editDigest({ enabled: event.target.checked })} /><span><strong>生成每日日报</strong><small>按设定时区和时间生成并发布。</small></span></label><label className="switch-field"><input type="checkbox" checked={Boolean(digestDraft.notify)} disabled={!digestDraft.enabled} onChange={(event) => editDigest({ notify: event.target.checked })} /><span><strong>发布后发送通知</strong><small>通知包含可直接打开阅读页的链接。</small></span></label></div><div className="form-grid two"><label><span>生成时间</span><input type="time" value={digestDraft.daily_time || '20:00'} onChange={(event) => editDigest({ daily_time: event.target.value })} /></label><label><span>时区</span><input value={digestDraft.timezone || ''} list="timezone-list" onChange={(event) => editDigest({ timezone: event.target.value })} /></label><label><span>日报条目上限（自适应）</span><input type="number" min="1" max="100" value={digestDraft.item_limit ?? 50} onChange={(event) => editDigest({ item_limit: Number(event.target.value) })} /><small>实际数量按当天新闻综合权重自动决定；此值只作为安全上限。</small></label><label><span>候选观察上限</span><input type="number" min="1" max="5000" value={digestDraft.observation_limit ?? 5000} onChange={(event) => editDigest({ observation_limit: Number(event.target.value) })} /></label></div><label><span>公开阅读服务地址</span><input type="url" value={digestDraft.public_base_url || ''} onChange={(event) => editDigest({ public_base_url: event.target.value })} placeholder="https://eos.example.com" /><small>通知链接使用此 HTTPS 地址；启用日报通知时必须填写。</small></label><div className="dialog-actions"><button className="button primary" disabled={busy || !digestDirty} onClick={() => { void (async () => { if (await onSaveDigest(digestDraft) !== false) setDigestDirty(false) })() }}><Save size={16} />保存日报配置</button></div></div></section>
