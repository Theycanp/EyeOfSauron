...
function DigestReader({ digest, onBack }: { digest: DigestDetail; onBack: () => void }) {
  ...
  return <article className="digest-reader">
    ...
    <section className="digest-items">{(digest.items || []).map((item, itemIndex) => <article className="digest-item" key={item.cluster_key}><div className="digest-score"><strong>{item.score.toFixed(1)}</strong><span>综合分</span></div><div><div className="digest-item-title"><span className="status-pill neutral">[{itemIndex + 1}]</span><h3>{item.title}</h3>{item.handling === 'immediate' && <span className="status-pill info">即时通知</span>}<span className={`status-pill ${item.importance >= 4 ? 'warning' : 'neutral'}`}>重要度 {item.importance}</span></div><p>{item.summary}</p><div className="digest-tags">{[...(item.regions || []), ...(item.topics || [])].map((tag) => <span key={tag}>{tag}</span>)}</div>{(item.links || []).length > 0 && <div className="digest-links">{(item.links || []).map((link, index) => <a href={link} key={link} target="_blank" rel="noreferrer"><ExternalLink size={13} />来源 {index + 1}</a>)}</div>}</div></article>)}</section>
